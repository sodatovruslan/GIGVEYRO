...
export function AppealsPage({ role }: { role: UserRole }) {
  ...
  const [fieldErrors, setFieldErrors] = useState<AppealFieldErrors>({});
  const [saving, setSaving] = useState(false);

  const fieldMessages = { dealNotEligible: t("validation.dealNotEligible"), activeAppealExists: t("validation.activeAppealExists"), dealNotFound: t("validation.dealNotFound"), invalidMessage: t("validation.message"), invalidOwnerNote: t("validation.ownerNote") };

  async function mutate(action: () => Promise<unknown>, mapFieldErrors?: (reasonValue: unknown) => AppealFieldErrors) {
    setSaving(true); setError(""); setFieldErrors({});
    try { await action(); setDialog(null); setSelected(null); setMessage(""); await query.refetch(); }
    catch (reasonValue) {
  ...
  function resolveAppeal(event: FormEvent) { event.preventDefault(); if (selected) void mutate(() => appealsApi.resolve(selected.id, resolution, message), (reasonValue) => mapAppealFieldErrors(reasonValue, fieldMessages)); }
