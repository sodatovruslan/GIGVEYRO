...
export function DepositsPage({ owner }: { owner: boolean }) {
  ...
  const query = useApiQuery(() => depositsApi.list(owner, status || undefined, ownerFilters), `deposits:${owner}:${status}:${JSON.stringify(ownerFilters)}`);
  ...
  function applyFilters(event: FormEvent) { event.preventDefault(); setOwnerFilters(draft); }
  function resetFilters() { setDraft(emptyOwnerFilters); setOwnerFilters(emptyOwnerFilters); }
