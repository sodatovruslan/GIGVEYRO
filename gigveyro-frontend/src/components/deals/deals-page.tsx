...
export function DealsPage({ role }: { role: Exclude<UserRole, "team_lead"> }) {
  ...
  return <section>
    ...
    {(detailLoading || detail || detailError) && <Modal title={t("detailTitle")} close={closeDetail}>{detailLoading ? <p>{t("loading")}</p> : detailError ? <div className={styles.error}>{detailError}</div> : detail && <DealDetailGrid deal={detail} role={role} />}</Modal>}
  </section>;
