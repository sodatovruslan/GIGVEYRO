...
import type { NotificationItem, NotificationPreferences } from "@/lib/api/types";
...
export function NotificationsPage() {
  ...
  async function open(item: NotificationItem) {
    ...
    const link = account ? notificationLink(item, account.role) : null;
  ...
  return <section><div className={styles.heading}><div><span>{t("eyebrow")}</span><h1>{t("title")}</h1><p>{t("subtitle")}</p></div><button onClick={() => void perform(notificationsApi.markAllRead)}>{t("readAll")}</button></div>{error && <div className={styles.error}>{error}</div>}
    <div className={styles.layout}><div className={styles.feed}>{list.loading ? <div className={styles.state}>{t("loading")}</div> : list.error ? <div className={`${styles.state} ${styles.errorText}`}>{list.error}</div> : !list.data?.length ? <div className={styles.state}>{t("empty")}</div> : <>{list.data.map((item) => <button key={item.id} className={item.is_read ? styles.read : ""} onClick={() => void open(item)}><i /><div><span>{labels.notification(item.type)}</span><h2>{item.title}</h2><p>{item.message}</p><small>{format.dateTime(item.created_at)}</small></div></button>)}<Pager offset={offset} limit={PAGE_SIZE} itemCount={list.data.length} onPage={setOffset} /></>}</div>
