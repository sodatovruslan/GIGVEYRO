...
import { ownerUserWithdrawalsApi } from "@/lib/api/owner-user-withdrawals";
...
export default function OwnerUserWithdrawalsPage() {
  ...
  const query = useApiQuery(() => ownerUserWithdrawalsApi.list(undefined, PAGE_SIZE, offset), `owner-user-withdrawals:${offset}`);
  const [error, setError] = useState("");
  const [busyId, setBusyId] = useState<string | null>(null);

  async function approve(id: string) {
    setError(""); setBusyId(id);
    try { await ownerUserWithdrawalsApi.approve(id, null); await query.refetch(); }
  ...
  async function reject(id: string) {
    setError(""); setBusyId(id);
    try { await ownerUserWithdrawalsApi.reject(id, null); await query.refetch(); }
    catch (reason) { setError(localizeError(reason)); }
    finally { setBusyId(null); }
  }
