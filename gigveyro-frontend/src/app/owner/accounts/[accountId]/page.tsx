...
export default function OwnerAccountDetailsPage() {
  ...
  function open(nextDialog: Dialog) { setMutationError(""); setAmount(""); setDescription(""); setPassword(""); setDialog(nextDialog); }
  ...
  const dialogTitle = (value: Exclude<Dialog, null>) => ({ edit: t("editTitle"), password: t("passwordTitle"), allocate: t("allocateTitle"), insurance: t("insuranceTitle"), adjust: t("adjustTitle") })[value];
  ...
  return <section>
    ...
    {account.role !== "owner" && <><div className={styles.sectionTitle}><div><span>{t("finance")}</span><h2>{isMerchant ? t("merchantWallet") : t("userWallet")}</h2></div>{!isMerchant && <div className={styles.walletActions}><button onClick={() => open("allocate")}>{t("allocate")}</button><button onClick={() => open("insurance")}>{t("insurance")}</button><button onClick={() => open("adjust")}>{t("adjust")}</button></div>}</div>
  ...
  function WalletCard({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) { return <article className={styles.walletCard}><span>{label}</span><strong className={accent ? styles.accent : ""}>{format.number(value)}</strong><small>USDT</small></article>; }
