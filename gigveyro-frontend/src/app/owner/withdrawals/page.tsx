...
export default function OwnerWithdrawalsPage() {
  ...
  async function act() {
    ...
    try {
      ...
      else if (target.action === "reject") await ownerOperationsApi.rejectWithdrawal(target.item.id, value);
      else await ownerOperationsApi.markWithdrawalPaid(target.item.id, value);
