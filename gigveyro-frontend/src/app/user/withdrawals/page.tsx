...
export default function UserWithdrawalsPage() {
  ...
  const query = useApiQuery(() => userApi.withdrawals(undefined, PAGE_SIZE, offset), `user-withdrawals:${offset}`);
  ...
  const [detail, setDetail] = useState<UserWithdrawal | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailError, setDetailError] = useState("");

  async function openDetail(id: string) {
    setDetail(null); setDetailError(""); setDetailLoading(true);
    try { setDetail(await userApi.getWithdrawal(id)); }
  ...
  async function create(event: FormEvent) {
    ...
    try {
      await userApi.createWithdrawal({ amount, destination_type: "usdt_trc20_address", destination, comment: comment || null });
  ...
  async function cancel(id: string) {
    setError("");
    try { await userApi.cancelWithdrawal(id); await query.refetch(); }
    catch (reason) {
  ...
  return <section>
    <div className={styles.contentHeader} style={{ padding: 0, border: 0, marginBottom: 27 }}>
      <PageHeading eyebrow={t("eyebrow")} title={t("userTitle")} text={t("userSubtitle")} />
