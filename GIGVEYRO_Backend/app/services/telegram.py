...
class TelegramService:
    ...
    async def generate_link_code(self, account_id: uuid.UUID) -> TelegramAccountLink:
        code = secrets.token_hex(16)
        expires_at = datetime.now(timezone.utc) + timedelta(minutes=15)
        return await self.telegram_repo.create_or_update_link_code(account_id, code, expires_at)
    ...
    async def process_telegram_command(
        ...
        if cmd == "/start" and len(parts) > 1:
            code = parts[1]
            link = await self.telegram_repo.get_by_link_code(code)
            if not link:
                return "Invalid or expired link code."
            if link.link_expires_at and link.link_expires_at < datetime.now(timezone.utc):
