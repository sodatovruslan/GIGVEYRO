...
class TeamLeadWithdrawalCreate(BaseModel):
...
class OwnerTeamLeadWithdrawalAction(BaseModel):
    comment: str | None = Field(default=None, max_length=500)


class TeamLeadWithdrawalRead(BaseModel):
...
class TeamLeadWithdrawalListResponse(BaseModel):
    items: list[TeamLeadWithdrawalRead]
    total: int
    limit: int
    offset: int
