...
class UserWithdrawalCreate(BaseModel):
...
class OwnerUserWithdrawalAction(BaseModel):
    comment: str | None = Field(default=None, max_length=500)


class UserWithdrawalRead(BaseModel):
...
class UserWithdrawalListResponse(BaseModel):
    items: list[UserWithdrawalRead]
    total: int
    limit: int
    offset: int
