...
class TeamLeadWithdrawal(Base):
    """A TEAM_LEAD's direct withdrawal of their own accrued profit - same
    status/approval shape as UserWithdrawal (kept as a separate table/
    service rather than generalizing UserWithdrawal, matching that model's
    own precedent for a second caller with distinct semantics)."""

    __tablename__ = "team_lead_withdrawals"
    __table_args__ = (
        CheckConstraint("amount > 0", name="ck_team_lead_withdrawals_amount_positive"),
    )
    ...
    team_lead_id: Mapped[uuid.UUID] = mapped_column(
