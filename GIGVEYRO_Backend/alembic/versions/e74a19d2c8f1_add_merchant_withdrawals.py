...
down_revision: Union[str, None] = 'd83e29f1b2c4'
...
def upgrade() -> None:
    ...
    op.add_column('merchant_wallets', sa.Column('held_balance', sa.Numeric(precision=20, scale=8), server_default='0', nullable=False))
    op.create_check_constraint('ck_merchant_wallets_held_non_negative', 'merchant_wallets', 'held_balance >= 0')

    # 3. Add held columns to ledger_entries
    op.add_column('ledger_entries', sa.Column('held_before', sa.Numeric(precision=20, scale=8), server_default='0', nullable=False))
    op.add_column('ledger_entries', sa.Column('held_after', sa.Numeric(precision=20, scale=8), server_default='0', nullable=False))

    # 4. Create ENUMs for withdrawals
    op.execute("CREATE TYPE withdrawal_status AS ENUM ('pending', 'approved', 'paid', 'rejected', 'cancelled')")
    op.execute("CREATE TYPE withdrawal_destination_type AS ENUM ('usdt_trc20_address', 'bybit_uid')")
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['merchant_wallet_id'], ['merchant_wallets.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_merchant_withdrawals_created_at'), 'merchant_withdrawals', ['created_at'], unique=False)
    op.create_index(op.f('ix_merchant_withdrawals_merchant_id'), 'merchant_withdrawals', ['merchant_id'], unique=False)
    op.create_index(op.f('ix_merchant_withdrawals_merchant_wallet_id'), 'merchant_withdrawals', ['merchant_wallet_id'], unique=False)
    op.create_index(op.f('ix_merchant_withdrawals_public_id'), 'merchant_withdrawals', ['public_id'], unique=True)
    op.create_index(op.f('ix_merchant_withdrawals_status'), 'merchant_withdrawals', ['status'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_merchant_withdrawals_merchant_wallet_id'), table_name='merchant_withdrawals')
