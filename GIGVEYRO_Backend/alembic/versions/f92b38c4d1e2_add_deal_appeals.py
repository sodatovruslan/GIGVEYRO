...
down_revision: str | None = "e74a19d2c8f1"
...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE TYPE appeal_status AS ENUM ('open', 'under_review', 'resolved', 'cancelled')"
    )
    op.execute(
        "CREATE TYPE appeal_reason AS ENUM "
        "('payment_not_received', 'wrong_amount', 'payment_proof_issue', "
        "'timeout_dispute', 'other')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "opened_by_role",
            postgresql.ENUM("owner", "user", "merchant", name="user_role", create_type=False),
            nullable=False,
        ),
        sa.Column(
            "reason_code",
        ...
        sa.Column(
            "resolution",
            postgresql.ENUM(
                "settle_to_merchant", "release_to_user", name="appeal_resolution", create_type=False
            ),
            nullable=True,
        ),
    ...
    op.create_index(
        op.f("ix_deal_appeals_created_at"), "deal_appeals", ["created_at"], unique=False
    )
    op.create_index(op.f("ix_deal_appeals_deal_id"), "deal_appeals", ["deal_id"], unique=False)
    op.create_index(
        op.f("ix_deal_appeals_opened_by_account_id"),
        "deal_appeals",
        ["opened_by_account_id"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_deal_appeals_reason_code"), "deal_appeals", ["reason_code"], unique=False
    )
