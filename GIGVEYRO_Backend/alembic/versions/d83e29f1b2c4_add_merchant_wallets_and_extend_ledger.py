...
down_revision: str | None = "cdd659866190"
...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_merchant_wallets_account_id"), "merchant_wallets", ["account_id"], unique=True
    )

    # 3. Alter ledger_entries table
    op.alter_column(
        "ledger_entries", "wallet_id", existing_type=postgresql.UUID(as_uuid=True), nullable=True
    )
    op.add_column(
        "ledger_entries",
        sa.Column("merchant_wallet_id", postgresql.UUID(as_uuid=True), nullable=True),
    )
    op.create_index(
        op.f("ix_ledger_entries_merchant_wallet_id"),
        "ledger_entries",
        ["merchant_wallet_id"],
        unique=False,
    )
    op.create_foreign_key(
        "fk_ledger_entries_merchant_wallet_id",
        "ledger_entries",
        "merchant_wallets",
        ["merchant_wallet_id"],
        ["id"],
    ...
    op.create_unique_constraint(
        "uq_ledger_entries_merchant_idempotency_key",
        "ledger_entries",
        ["merchant_wallet_id", "idempotency_key"],
    )


def downgrade() -> None:
    op.drop_constraint(
        "uq_ledger_entries_merchant_idempotency_key", "ledger_entries", type_="unique"
    )
    ...
    op.alter_column(
        "ledger_entries", "wallet_id", existing_type=postgresql.UUID(as_uuid=True), nullable=False
    )
